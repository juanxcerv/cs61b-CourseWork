...
class Game {
    ...
    ArrayList<Game> allDisplays = new ArrayList<Game>();
    ...
    /** Allows to go back to a previous state in the game. */
    void undo() {
        int x = allDisplays.size();
        if (x > 1) {
            copyFrom(allDisplays.remove(x - 1));
        }
