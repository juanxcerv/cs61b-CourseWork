...
public class Gitlet2 {
    ...
    void reset(String commitID) {
     ...
     for (String name : currCommit.getBlobs().getName()) {
        String hash = currCommit.getBlobs().get(name);  
        File blobFile = deserializeBlob(".gitlet/blobs/" + hash + ".ser");
        if (blobFile != null) {
    ...
    File commits = new File(".gitlet/commits");
    Commit resetBlob = deserializeCommit(".gitlet/commits/" + commitID + ".ser");
    File wd = new File(".");
    for (File file : wd.listFiles()) {
        ...
        if (resetBlob.getBlobs().get(name) != null) {
            String resetHash = resetBlob.getBlobs().getName();
            String wdHash = Utils.sha1(file.getName(), Utils.readContents(file));
            if ()
        }
    ...
    void branchCheckout(String branchName) {
        ...
        for (File file : wd.listFiles()) {
           ...
           String hash = next.getBlobs().get(file.getName());
            File check = new File(".gitlet/blobs/" + Utils.sha1(file.getName(),
                Utils.readContents(file)) + ".ser");
            if (!check.exists() && next != null) {
                System.out.println("There is an untracked file in the way; delete it or add it first.");
                return;
            }
        }
